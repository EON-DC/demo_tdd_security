...
public class ShippingJpaStore  implements  ShippingStore{
    ...
    public ShippingJpaStore(ShippingAddressJpaRepository jpaRepository) {
    ...
    public List<ShippingAddress> getAll() {
        return jpaRepository.findAll()
                .stream().map(ShippingAddressEntity::toDomain)
    ...
    public ShippingAddress get(String id) {
        return jpaRepository.findById(id).orElseThrow(
                () -> new RuntimeException("No such shipping id : " + id)
        ).toDomain();
    }
